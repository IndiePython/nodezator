...
def _chain(*iterables:Iterable) -> 'iterator': pass
...
def _dumps(
    ...
    ) -> [{'name': 'string', 'type': str}]: pass

def _get_iterable_return_iterator(iterable:Iterable) -> (
  'iterator'
): pass
...
def _count(start:(int, float)=0, step:(int, float)=1) -> (
  'iterator'
): pass
...
def _groupby(iterable:Iterable, key:Callable=None) -> (
  'iterator'
): pass
...
def _partial(func:Callable, *args, **keywords) -> [
...
def _reduce1(function:Callable, iterable:Iterable): pass
...
def _repeat1(obj) -> 'iterator': pass
...
STLIB_IDS_TO_SIGNATURE_CALLABLES_MAP = {
  ...
  'literal_eval(node_or_text_string)' : (
    _literal_eval2
  ),
...
STLIB_IDS_TO_SIGNATURES_MAP = {
  ...
  'literal_eval(node_or_string)' : (
    signature(_literal_eval1)
  ),

  'literal_eval(node_or_text_string)' : (
    signature(_literal_eval2)
  ),
...
STLIB_IDS_TO_SIGNATURES_MAP = {
  ...
  for stlib_id, callable_obj
  in STLIB_IDS_TO_SIGNATURE_CALLABLES_MAP.items()
