...
class MenuSetup:
    ...
    def create_menubar(self):
            ...
            'children': [
...

              {
                ...
                'command': (
                  partial(
                    open_htsl_link,
                    'htap://nodezator.pysite/license.htsl'
                  )
                )
              },
...

              {
                ...
                'command': (
                  partial(
                    open_htsl_link,
                    'htap://nodezator.pysite/about.htsl'
                  )
                )
              },
        ...
        # menu
            ...
            graph_menu = {
              ...
              'children': [
                ...
                {
                  'label' : (
                    t.menu.change_node_packs_on_file
                  ),
                  'command' : (
                    present_change_node_packs_form
                  ),
                ...
                {
                  'label' : (
                    t.menu.rename_node_packs_on_file
                  ),
                  'command' : (
                    present_rename_node_packs_form
                  ),
        ...
        # adding new menus/commands
            ...
            file_children_data.insert(
                                 4, save_command_data
                               )

            file_children_data.insert(
                                 5, save_as_command_data
                               )
            ...
            graph_menu = {
              ...
              'children': [
                ...
                {
                  'label' : (
                    t.menu.change_category_colors
                  ),
                  'command' : (
                    APP_REFS.ea.change_category_colors
                  ),
                },

                {
                  'label' : (
                    t.menu.select_node_packs
                  ),
                  'command' : (
                    APP_REFS.ea.select_node_packs
                  ),
                },
                ...
                {
                  'label' : (
                    t.menu.execute_with_custom_stdout
                  ),
                  'key_text' : "Shift+F12",
                  'command' : (
    ...
    def create_canvas_popup_menu(self):
        ...
        for preset_name, widget_data in (
          WIDGET_DATA_PRESET_MAP.items()
        ):

            label_text  = (
              WIDGET_PRESET_MENU_LABEL_MAP[preset_name]
            )
        ...
        module_name_id_pairs = sorted(
          ...
          (
            ...
            for stlib_id, module_obj
            in STLIB_IDS_TO_MODULE.items()
