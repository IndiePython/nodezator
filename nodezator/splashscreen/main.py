...
class SplashScreen(SplashScreenOperations):
        ...
        self.subheading_label = \
          Object2D.from_surface(
            surface=render_text(
                   ...
                   text = (
                     t.splash_screen.nodes_from_functions
                   ),
                   ...
                   **{
                     **TEXT_SETTINGS,
                     'font_height': SUBHEADING_FONT_HEIGHT
                   }
                 )
          )
    ...
    def set_animation_support_objects(self):
        ...
        width, height = (
          self.animation_objects.rect.inflate(30, 40).size
        )
    ...
    def position_and_define_boundaries(self):
        ...
        self.title_label.rect.bottomleft = (
          self.app_icon.rect.move(5, 0).bottomright
        )
        ...
        self.kind_label.rect.bottomleft = (
          self.title_label.rect.move(0, 8).topleft
        )

        header.append(self.kind_label)

        self.subheading_label.rect.midtop = (
          header.rect.midbottom
        )
        ...
        self.release_level_label.rect.midleft = (
          self.kind_label.rect.move(10, 0).midright
        )
        ...
        file_related_objs = List2D(
                              self.action_objs
                              + self.recent_file_objs
                            )

        if self.recent_file_objs:
            
            self.recent_file_objs.rect.topleft = (
              self.action_objs
              .rect.move(0, 10).bottomleft
            )

        file_related_objs.rect.topleft = (
          body.rect
          .move(20, 0).topright
        )
        ...
        ##
        self.project_link_objs.rect.topleft = (
          body.rect
          .move(20, 0).topright
        )
