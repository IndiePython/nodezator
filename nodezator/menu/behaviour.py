...
class BehaviourDefinitions(Object2D):
    ...
    def define_drawing_behaviours(self):
        ...
        if self.is_scrollable():
            ...
            # area
            self.draw_top_items.append(
                            self.draw_items_on_scroll_area)
    ...
    def draw_items_on_scroll_area(self):
        ...
        ### self.rect.topleft)
        offset = invert_point(self.rect.topleft,
    ...
    def highlight_widget_and_parents(self, widget):
        ...
        try: self.highlight_widget_and_parents(widget.parent)
    ...
    def collapse_active_menu(self):
        ...
        try: self.draw.remove(self.draw_active_branch)
        except ValueError: pass

        ### check existence of unhighlight method
        try: method = self.active_menu.unhighlight
    ...
    def get_top_menu(self, widget):
        ...
        if self.is_top_menu(widget): return widget

        ### otherwise check its parent
        else: return self.get_top_menu(widget.parent)
    ...
    def reposition_children_bodies(self):
        ...
        for item in self.children:
            ...
            try: method = item.reposition_body
    ...
    def warp(self, mouse_pos):
        ...
        if 'right'  in attr_name: dx += 1
        if 'bottom' in attr_name: dy += 1
        ...
        if self.is_scrollable():
            ...
            if self.is_menubar:
                ...
                self.scroll_area.topleft = \
                            self.left_arrow.rect.topright

                self.right_arrow.rect.topleft = \
                                self.scroll_area.topright
